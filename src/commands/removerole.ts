...
export = {
  help: {
    name: "Removerole",
    usage: "removerole @roles | @user @roles",
    description: "Remova cargos de um membro",
    aliases: ["rmrole"],
    permissions: 0,
  ...
  run: async (client:Client, message:Message, args) => {
    ...
    .setDescription(`
      **Os cargos**
      \`\`${await roles.map(e => { if(!member.roles.cache.has(e.id)) return; member.roles.remove(e.id).catch(message.channel.send); return e.name }).join(", ")}\`\`
      Foram removidos do membro: ${member.user.username}
    `)
